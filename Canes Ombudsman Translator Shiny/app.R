...
translate_to_ombudsman <- function(text) {
  ...
  ui <- fluidPage(
    textInput(width = "100%","caption", "Canes Ombudsman Translator", "Boo Nieves gives me a good vibe, in my opinion."),
    verbatimTextOutput("value")
  )
  server <- function(input, output) {
    output$value <- renderText({ translate_to_ombudsman(input$caption) })
  }
  shinyApp(ui, server)
